      ...
      <FormControl variant="standard" sx={{ m: 1, minWidth: { width } }}>
        {items ? (
          <CustomSelect
            ...
            id="dropdown-select"
            value={selected?.value || ''} // Ensure it's using the 'value' property of the selected object
          ...
          >
            {items &&
              items.map((item, idx) => (
                <Option
                  key={item?.value} // Use a unique key, like item.value or item.id
                  value={item.value} // Use the value for selection
                  color={color}
                  bgcolor={bgcolor}
                >
                  {item?.title ?? item}
