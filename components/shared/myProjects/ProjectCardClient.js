...
const StyledCard = styled.div`
  ...
  position: relative;
  opacity: ${(props) =>
    props.status === 'cancelled' || props.status === 'completed' ? '0.7' : '1'};
    ...
    <StyledCard status={getStatus(projectDetail.attributes.status)}>
      <Row sx={{ justifyContent: 'center' }}>
        <StyledPill status={getStatus(projectDetail.attributes.status)}>
          <StatusText>
            {' '}
            {getStatus(projectDetail.attributes.status)}{' '}
          </StatusText>
        </StyledPill>
      </Row>
      <Row>
      ...
      <Row sx={{ justifyContent: 'center' }}>
        <Text color="gray" size="small">
          Project ID {projectDetail.attributes.id}
        </Text>
      </Row>
      <VerticalDivider />
      <Row sx={{ justifyContent: 'space-between' }}>
        <Text>Price</Text>
        <Text color="green">{projectDetail.attributes.maxBudget} USD</Text>
      </Row>
      <Row sx={{ justifyContent: 'space-between' }}>
        <Text>Location</Text>
        <Text color="green">
          {projectDetail.attributes.city ??
            '' + projectDetail.attributes.country}
        </Text>
      </Row>
      <Row sx={{ justifyContent: 'space-between' }}>
        <Text>Date</Text>
        <Text color="green">
          {formatDate(projectDetail.attributes.deliveryDate)}
        </Text>
      </Row>
      <VerticalDivider />
      <Row sx={{ justifyContent: 'center' }}>
